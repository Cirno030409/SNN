...
def main():
    ...
    ## ニューロングループの定義
    N["inp"] = PoissonGroup(num["inp"], 10*Hz) # 入力ニューロン
    N["exc"] = NeuronGroup(num["exc"], eqs, threshold='v>v_th', reset='v=v_re', refractory=ref, method='euler') # 興奮性ニューロン
    N["inh"] = NeuronGroup(num["inh"], eqs, threshold='v>v_th', reset='v=v_re', refractory=ref, method='euler') # 抑制性ニューロン
